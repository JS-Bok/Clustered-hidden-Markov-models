...
#### EM for MPLE ####
EM_MPLE_singleLambda <- function(y,A=NULL,dist_class,parameter=NULL,Lambda, tol=1e-5, m=NULL){
  ...
  EM_MPLE_step <- function(x, y, A, dist_class, parameter, Lambda, A_hat, prob_prev=NULL){
    ...
    for (ad in c(1:100)) { 
      ...
      for (i in c(1:(m*(m-1)/2))) {
        ...
        a[(1+(m-1)*(i-1)):((m-1)*i)] <- update_a(GTO_variable[(1+(m-1)*(i-1)):((m-1)*i)], rho,gamma,Lambda)
      }
    ...
    for (i in c(1:(m*(m-1)/2))) {
      Pen <- Pen + SCAD_pen(n2_sqrt(SCAD_variable[(1+(m-1)*(i-1)):((m-1)*i)]), Lambda, gamma)
    }
...
EM_MPLE <- function(y,
  ...
  threshold <- 0.5 * log(tot_time) * (m-1)^2  # 임계값 계산
    ...
    for (lam in Lambda_vec) {
      tmp <- tryCatch({
        EM_MPLE_singleLambda(y=y, A=initial_hmm$transition, dist_class=dist_class, parameter=initial_hmm$par, lam, tol = tol)
  ...
  if (is.null(CHMM_result)) {
    stop("CHMM fitting failed for all lambda values")
  }
