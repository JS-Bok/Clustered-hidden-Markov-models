...
ordered_pairs_diffmat <- function(m){
  pairs <- matrix(NA_integer_, nrow = m*(m-1)/2, ncol = 2)
  idx <- 1L
  for (i in 1:(m-1)) {
    for (k in (i+1):m) {
      pairs[idx, ] <- c(i, k)
...
block_idx <- function(l, B) (1 + B*(l-1)):(B*l)
...
# Gradient function for the penalized estimation
...
hessian_nonpen <- function(A_vec, V_sum, LB_par,m,tot_time) {
  ...
  # A_vec -> m x (m-1)
  A_small <- matrix(A_vec, nrow = m, byrow = TRUE)
  A_last  <- 1 - rowSums(A_small)
  
  # d: m x q,  c: length m
  d <- (V_sum[, 1:q, drop = FALSE] / tot_time + invLB) / (A_small * A_small)
  c <- (V_sum[, m] / tot_time + invLB) / (A_last * A_last)
  ...
  for (i in seq_len(m)) {
    ...
    # block fill: off-diagonal c_i
    hess[idx, idx] <- c[i]
    
    # diagonal only: + d_i
    diag_pos <- idx + (idx - 1L) * p  # row + (col-1)*nrow, here: row=col=idx
    hess[diag_pos] <- hess[diag_pos] + d[i, ]
  }
...
hessian <- function(A_vec, V_sum, rho, LB_par,m,tot_time) {
  ...
  # 1) Likelihood + barrier: row-wise blocks
  for (i in seq_len(m)) {
    idx <- ((i - 1L) * q + 1L):(i * q)
    hess[idx, idx] <- c[i]
    
    diag_pos <- idx + (idx - 1L) * p
    hess[diag_pos] <- hess[diag_pos] + d[i, ]
  }
  
  # 2) ADMM quadratic: for each column j, add rho*(mI - J)
  if (rho != 0) {
    for (j in seq_len(q)) {
      idx <- seq.int(j, by = q, length.out = m)  # same column across rows
      
      # -rho * J 
      hess[idx, idx] <- hess[idx, idx] - rho
      
      # +rho * m * I
      diag_pos <- idx + (idx - 1L) * p
      hess[diag_pos] <- hess[diag_pos] + rho * m
    }
  }
...
stationary_dist <- function(x) {
  obj <- -t(x)+1+diag(nrow(x))
  inv <- tryCatch({Matrix::solve(obj)}, error = function(e) {
            tryCatch({chol2inv(chol(obj))},
                          error = function(e) { tryCatch({armaInv(obj)},
                                                         error = function(e) { return(inv_svd(obj)) }
                          ) }
          ) }
  )
  inv %*% matrix(1, nrow=nrow(x), ncol=1)
...
mvnorm_inv <- function(X,mean,sigma_inv) {
  ...
  log_det_inv <- log(det(sigma_inv))
  if(!is.nan(log_det_inv)){
    if(log_det_inv == Inf){
      log_det_inv <- sum(log(diag(chol(sigma_inv))))
    }
  }
  return(exp(-(k/2)*log(2*pi) -1/2*(t(junk) %*% sigma_inv %*% (junk)) +1/2*log_det_inv))
}
...
cluster_from_a <- function(a, m, eps_fuse = .Machine$double.eps){
  B <- m - 1
  pairs <- ordered_pairs_diffmat(m)
  ...
  for (l in 1:nrow(pairs)) {
    idx <- (1 + B*(l-1)):(B*l)
    if (sqrt(sum(a[idx]^2)) <= eps_fuse) {
      unionp(pairs[l,1], pairs[l,2])
    }
  }
  
  roots <- vapply(1:m, findp, numeric(1))
  split(1:m, roots)
}

refit_A_by_groups_mean <- function(A, groups, eps = .Machine$double.eps, renormalize = TRUE) {
  A_new <- A
  m <- ncol(A)
  
  for (g in groups) {
    
    rowG <- colMeans(A[g, , drop = FALSE])
    ...
    A_new[g, ] <- matrix(rowG, nrow = length(g), ncol = m, byrow = TRUE)
  }
...
HMM_simulation <- function(dimension, init_dist=NULL, transition, emission, size, return_X = FALSE){
  m <- nrow(transition)
  ...
  if(return_X){
    return(list(Y=observation, X=markov_chain))
  }
